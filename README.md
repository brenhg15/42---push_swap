*This project has been created as part of the 42 curriculum by bhernand and mdiakono.*
...
For larger inputs with low disorder (the adaptive trigger), the algorithm performs an insertion-sort-like pass: it scans consecutive pairs in A, pushes any element that is out of order to B, and once the pass is complete, merges the elements back from B into their correct positions in A.
...
- [Visualgo — Sorting visualizations](https://visualgo.net/en/sorting) — Used to verify the logical correctness of the chunk-based and radix approaches
- [42 push_swap subject](https://cdn.intra.42.fr/pdf/pdf/96200/en.subject.pdf) — Official project specification
- [LSD Radix Sort explanation — CS Princeton](https://algs4.cs.princeton.edu/51radix/) — Reference for the bit-level radix approach applied to integer stacks

### AI Usage

Artificial intelligence tools (specifically Claude by Anthropic) were used during this project in the following ways:
...
- **Flag parser debugging:** AI helped identify that the `--adaptive` flag was not being recognised as a valid strategy when combined with `--bench`, due to argument parsing order. The fix was implemented by the learners.
...
|---------|---------------|
| **bhernand** | [List brenhg15's contributions here — e.g., complex radix sort implementation, coordinate compression, stack operations layer, Makefile, input validation and error handling] |
| **mdiakono* | Simple algorithm (≤5 hardcoded sort + insertion-sort pass), medium chunk sort (partition and reconstruction phases), adaptive routing logic, disorder metric implementation (inversion count), benchmarking system (`--bench` flag and all 11 operation counters) |
